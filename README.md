...
Source for the Wickra Backtest site (**[backtest.wickra.org](https://backtest.wickra.org)**): hero, per-language API overviews, benchmarks and the in-browser indicator demo.
...
**Wickra Backtest** — streaming-native, event-driven backtester where a backtest and a live run over the same JSON spec are byte-identical. The library itself is
[`wickra-lib/wickra-backtest`](https://github.com/wickra-lib/wickra-backtest); its README carries the install line and a quick start for every language.
...
- **The released version** — the version menu in the nav, the Maven snippet on the landing page and the Java API page — is pulled hourly from [wickra-backtest's latest release](https://github.com/wickra-lib/wickra-backtest/releases/latest) by `sync-from-backtest.yml`.
